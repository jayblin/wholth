...
concept is_std_vector =
    is_std_container<T> && requires(T t) { t.emplace_back(); };
...
concept is_inherently_serializable =
    requires(Value v, Serializer& serializer_ref) {
        v.serialize(serializer_ref);
    };

template <typename Value, typename Serializer>
concept is_convertably_serializable = requires(
                                          const Value& value_ref,
                                          Serializer& serializer_ref,
                                          Serializable<Value> si) {
