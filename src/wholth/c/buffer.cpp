...
{
    return wholth::internal::ring_pool::fetch<wholth_Buffer>(buf);

    // assert(
    //     nullptr != buf && nullptr == *buf &&
    ...
    //     // todo add code enum
    //     err = {.code = 1, .message = {.data = msg.data(), .size =
    //     msg.size()}};
    // }
    // else
    // {
...
{
    return wholth::internal::ring_pool::unfetch<wholth_Buffer>(buf);
    // if (nullptr == buf)
    // {
    //     constexpr std::string_view msg = "Buffer is null!";
    //     // todo add code enum
    //     return {.code = 1, .message = {.data = msg.data(), .size =
    //     msg.size()}};
    // }
    //
