...
static void setup_db_global_options(wholth::Context& ctx)
{
    ...
    if (!did_global_db_setup) {
        const std::error_code ec = sqlw::status::Code{sqlite3_config(SQLITE_CONFIG_LOG, error_log_callback, &ctx)};
        assertm(sqlw::status::Condition::OK == ec, "Error during sqlite3_config()");
...
) noexcept(false)
{
    ...
    if (!does_db_file_exist(db_path))
    {
        ...
        if (db::status::Condition::OK != migrate_result.error_code) {
            setup_db_cleanup(db_path);
            throw std::system_error(migrate_result.error_code, migrate_result.problematic_migration);
        }
