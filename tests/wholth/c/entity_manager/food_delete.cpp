...
class Test_wholth_em_food_delete : public ApplicationAwareTest
{
};

TEST_F(Test_wholth_em_food_delete, when_good_case)
{
    auto& con = db::connection();

    std::string id;
    astmt(con, "INSERT INTO food (id, created_at) VALUES (555550, 'now')");
    // astmt(con, "INSERT INTO food_localisation (id, created_at) VALUES (555550, 'now')");

    wholth_Food food = wholth_entity_food_init();
    food.id = wtsv("22");

    auto buff = wholth_buffer_ring_pool_element();
    auto err = wholth_em_food_delete(&food, buff);
